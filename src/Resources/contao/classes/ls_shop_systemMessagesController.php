...
class ls_shop_systemMessagesController
{
	public function getIncompleteInstallationMessage() {
		$obj_installerController = \System::importStatic('ls_shop_installerController');

		// nichts machen, wenn Version niedriger als 2.11
		if (version_compare(VERSION, '2.11', '<')) {
			ob_start();
			?>
			<div class="ls_shop ls_shop_systemMessage versionNotCompatible">
				<?php echo sprintf($GLOBALS['TL_LANG']['MSC']['ls_shop']['systemMessages']['installToolMessage09']); ?>
			</div>
			<?php
			return ob_get_clean();
		}
		...
		if (!is_array($arrInstallationStatus) && $arrInstallationStatus == 'invalidSystemStatus') {
			...
			$objShopInstallationPage = \Database::getInstance()->prepare("
				SELECT		*
				FROM		`tl_page`
				WHERE		`alias` = 'merconis-installer-redirection'
					AND		`published` = '1'
			")
			->execute();
			
			if ($objShopInstallationPage->numRows) {
				$urlToFrontendShopInstallation = \Controller::generateFrontendUrl($objShopInstallationPage->row(), null, \PageModel::findWithDetails($objShopInstallationPage->id)->language);
			} else {
					...
					if (!$varUpdateSituation['blnHasError']) {
										...
										<p>
											...
											<?php
												...
												if ($obj_installerController->checkForNecessaryInstructions($lastVersionInTrail, $strVersion)) {
													?>
													&nbsp;&nbsp;&nbsp;<a target="_blank" href="http://www.merconis.com/tl_files/EigeneDateien/DokumenteUndSonstigeDateien/updateInstructions/index.htm#update_from_<?php echo preg_replace('/[.\s]/', '-', $lastVersionInTrail); ?>_to_<?php echo preg_replace('/[.\s]/', '-', $strVersion); ?>.htm"><?php echo $GLOBALS['TL_LANG']['MSC']['ls_shop']['systemMessages']['installToolMessage12']; ?></a>
													<?php
												} else {
												...
												if ($arrConverterRoutineInfo['routineName']) {
													if (!$arrConverterRoutineInfo['alreadyExecuted']) {
														if (!$arrConverterRoutineInfo['blnIsAllowed']) {
															...
															?>
															&nbsp;&nbsp;&nbsp;<a href="contao/main.php?lsShopUpdateAction=<?php echo $arrConverterRoutineInfo['routineName'];?>"><?php echo $GLOBALS['TL_LANG']['MSC']['ls_shop']['systemMessages']['installToolMessage18']; ?></a>
															<?php
														}
													} else {
														?>
						...
						if ($varUpdateSituation['errorCode'] == 'installedVersionUnknown') {
							...
							foreach ($varUpdateSituation['arrVersionHistory'] as $versionInHistory) {
								...
								?>
								<a href="contao/main.php?lsShopUpdateAction=setInstalledVersion&installedVersion=<?php echo urlencode($versionInHistory); ?>"><?php echo $versionInHistory; ?></a><br />
								<?php
							}
						} else {
		...
		/*
		 * Wenn der Sprachwechsler, der ja eine Pflichtkomponente ist, noch nicht installiert ist,
		 * so befindet sich der Installer zwangsläufig noch ganz am Anfang
		 */
		if (!$arrInstallationStatus['languageSelectorInstalled']) {
			ob_start();
			?>
			<div class="ls_shop ls_shop_systemMessage">
				<?php echo sprintf($GLOBALS['TL_LANG']['MSC']['ls_shop']['systemMessages']['installToolMessage01-01'], 'contao/main.php?lsShopInstallationStep=1'); ?>
			</div>
			<?php
			return ob_get_clean();
		}
		...
		if ($arrInstallationStatus['wholeDBOkay']) {
			...
			<div class="ls_shop ls_shop_systemMessage">
				...
				<div class="merconisThemeSourceSwitch">
					<div class="switch">
						<a onclick="this.blur();" class="<?php echo !isset($_SESSION['lsShop']['themeSource']) || $_SESSION['lsShop']['themeSource'] == 'local' ? 'active' : 'inactive'; ?>" href="contao/main.php?switchThemeSource=local"><?php echo $GLOBALS['TL_LANG']['MSC']['ls_shop']['systemMessages']['installToolMessage01-14']; ?></a>
						<a onclick="this.blur();" class="<?php echo isset($_SESSION['lsShop']['themeSource']) && $_SESSION['lsShop']['themeSource'] == 'repository' ? 'active' : 'inactive'; ?>" href="contao/main.php?switchThemeSource=repository"><?php echo $GLOBALS['TL_LANG']['MSC']['ls_shop']['systemMessages']['installToolMessage01-15']; ?></a>
					</div>
					<form action="contao/main.php?lsShopInstallationStep=2" method="post">
						...
						if (is_array($obj_installerController->availableThemes) && count($obj_installerController->availableThemes)) {
								...
								<div class="installerThemeSelection">
									<?php
										if ($arrTheme['compatibleWithContaoVersion'] && $arrTheme['compatibleWithMerconisVersion']) {
											?>
												<input<?php echo $arrTheme['default'] ? ' checked' : ''; ?> type="radio" name="installer_selectedTheme" id="installer_selectedTheme<?php echo $arrTheme['id']; ?>" value="<?php echo $arrTheme['id']; ?>|<?php echo $arrTheme['version']; ?>">
											<?php
										}
									?>
									<label <?php if ($arrTheme['compatibleWithContaoVersion'] && $arrTheme['compatibleWithMerconisVersion']) { ?> for="installer_selectedTheme<?php echo $arrTheme['id']; ?><?php } ?>">
										...
										?>
										<span class="preview"><a target="_blank" href="<?php echo $arrTheme['livePreviewUrl']; ?>"><?php echo $GLOBALS['TL_LANG']['MSC']['ls_shop']['systemMessages']['installToolMessage01-08']; ?></a></span>
									</label>
								</div>
								<?php
							}
							...
							?>
							<input type="submit" name="selectTheme" value="<?php echo $GLOBALS['TL_LANG']['MSC']['ls_shop']['systemMessages']['installToolMessage01-07']; ?>">
							<?php
						} else {
							?>
