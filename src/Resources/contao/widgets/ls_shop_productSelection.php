...
class ls_shop_productSelection extends \Widget
{
	...
	public function generate() {
		...
		<div class="widgetProductSelection">
			...
			<a onclick="ls_shop_productSelection.setCurrentlyActiveWidgetElement(getParent()); Backend.getScrollOffset();Backend.openModalIframe({'width':765,'title':'<?php echo specialchars(str_replace("'", "\\'", $GLOBALS['TL_LANG']['be_productSearch']['text008'])); ?>','url':this.href,'id':'<?php echo $this->strId; ?>'});return false;" href="<?= \System::getContainer()->get('router')->generate('merconis.backend.product_search', ['mode' => 'productSelection']) ?>" title="<?php echo specialchars($GLOBALS['TL_LANG']['be_productSearch']['text008']); ?>">
